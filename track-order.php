...
if ($token) {
    try {
        $db = DB::getInstance();
        $stmt = $db->prepare("SELECT * FROM orders WHERE id = ? OR tracking_token = ? OR tracking_number = ?");
    ...
    try {
        $db = DB::getInstance();
        $stmt = $db->prepare("SELECT * FROM orders WHERE (id = ? OR tracking_number = ?) AND email = ?");
        $stmt->execute([$order_number, $order_number, $email]);
        $result = $stmt->fetch();
        if ($result) {
            $order = $result;
        } else {
        ...
        <div class="bg-[#0a0a0a] border border-white/5 p-12 rounded-[3.5rem] shadow-2xl">
            <?php if (!$order): ?>
                <form action="track-order.php" method="GET" class="space-y-4 mb-8 pb-8 border-b border-white/5">
                    <div>
                        <label class="text-[9px] font-black uppercase tracking-widest text-white/20 block mb-4 ml-2">Secure Order or Tracking Number</label>
                        <input type="text" name="order_number" required placeholder="ORD-XXXXXX or TRACK-XXXX" class="w-full bg-white/[0.02] border border-white/5 p-4 rounded-xl text-white text-xs font-mono tracking-widest focus:border-amber-600 outline-none uppercase transition-all mb-4">
                        <label class="text-[9px] font-black uppercase tracking-widest text-white/20 block mb-4 ml-2">Authorization Email</label>
                    ...
                    <button type="submit" class="btn btn-gold w-full py-4 uppercase text-[10px] tracking-[0.4em] mt-4">Lookup via Credentials</button>
                </form>

                <form action="track-order.php" method="GET" class="space-y-4">
                    <div>
                        <label class="text-[9px] font-black uppercase tracking-widest text-white/20 block mb-4 ml-2">Or Use Verification Token / Tracking ID</label>
                    ...
                    <button type="submit" class="btn border border-white/10 text-white/40 hover:text-white w-full py-4 uppercase text-[10px] tracking-[0.4em]">Fast Track Lookup</button>
                    <p class="text-[9px] text-white/30 mt-6 text-center uppercase tracking-[0.2em] italic">Check your shipment confirmation for the 12-digit frequency token.</p>
                </form>
            <?php else: ?>
                <div class="space-y-12">
                    <div class="flex justify-between items-start border-b border-white/5 pb-10">
                        ...
                        <div class="text-right">
                            <span class="px-6 py-2 bg-amber-600/10 border border-amber-600/20 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-full"><?php echo strtoupper($order['status']); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 text-decoration-none">
                        <div>
                            ...
                            <div class="space-y-4">
                                <p class="text-sm font-bold uppercase tracking-widest text-white"><?php echo $order['carrier'] ?: $order['delivery_method']; ?></p>
                                <?php if ($order['tracking_number']): ?>
                                    <p class="text-[11px] font-mono text-amber-600 uppercase tracking-widest">Tracking: <?php echo e($order['tracking_number']); ?></p>
                                    <a href="<?php echo get_tracking_url($order['carrier'], $order['tracking_number']); ?>" target="_blank" class="inline-block mt-2 text-[9px] font-black uppercase tracking-widest text-amber-600 hover:text-white transition-colors">Carrier Status &rarr;</a>
                                <?php endif; ?>
